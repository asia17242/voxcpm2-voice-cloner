...
#   Intel Arc (XPU) -> pip install torch --index-url .../xpu + auto patch
#   No GPU (CPU)    -> pip install torch --index-url .../cpu
...
$uvCheck = powershell -Command "python -m uv --version" 2>$null
...
if (Test-Path $venvPython) {
    Write-Host "  $venvName already exists, skipping." -ForegroundColor Green
} else {
    python -m uv venv --python 3.12 $venvName
    Write-Host "  venv created: $venvName" -ForegroundColor Green
}
...
switch ($gpuType) {
    'cuda' {
        ...
        Write-Host '  -> Installing CUDA PyTorch' -ForegroundColor Green
        $torchIndex = 'https://download.pytorch.org/whl/cu128'
    }
    'xpu' {
        Write-Host "  Detected Intel Arc GPU: $gpuName" -ForegroundColor Green
        Write-Host '  -> Installing XPU PyTorch with auto patch' -ForegroundColor Green
        $torchIndex = 'https://download.pytorch.org/whl/xpu'
    }
    default {
        Write-Host '  No discrete GPU detected, using CPU mode.' -ForegroundColor Yellow
        Write-Host '  -> Installing CPU PyTorch (slower)' -ForegroundColor Yellow
...
if ($gpuType -eq 'xpu') {
    ...
    if (Test-Path $repatchScript) {
        ...
        Write-Host '  Intel Arc GPU requires manual patch execution to function.' -ForegroundColor Red
    }
} else {
    Write-Host '[6/6] No patch needed (not XPU mode).' -ForegroundColor Green
}
...
Write-Host '  1. Record voice: .\.venv\Scripts\python.exe record.py'
Write-Host '  2. Generate voice: .\.venv\Scripts\python.exe clone.py "Your text here"'
Write-Host ''

# Store GPU type for other scripts to read
